...
public MainWindow()
        {
            ...
            AppLogger.WriteLine("INIT", "Events subscribed.");
            UpdateColorPreview();
            AppLogger.WriteLine("INIT", "Color preview updated.");
            AppLogger.WriteLine("INIT", "MainWindow ready.");

            // Auto-start scan after window loads
            Loaded += async (s, e) =>
            {
                AppLogger.WriteLine("AUTO", "Window loaded, starting scan...");
        ...
        private void OnAutoConnected(object? sender, BleDeviceInfo device)
        {
            Dispatcher.Invoke(() =>
            {
                ...
                UpdateStatusBar($"Connected to {device.Name} ✓", true);
            });
        }
